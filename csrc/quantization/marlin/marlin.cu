...
std::optional<thread_override_t> get_turing_thread_override() {
  static bool initialized = false;
  static std::optional<thread_override_t> override_cfg;
  if (initialized) {
    return override_cfg;
  }
  initialized = true;

  const char* raw = std::getenv("VLLM_MARLIN_TURING_THREAD_CONFIG");
  ...
  int thread_n = -1;
  int num_threads = -1;
  if (std::sscanf(raw, "%d,%d,%d", &thread_k, &thread_n, &num_threads) != 3) {
    return std::nullopt;
  }

  int max_m = -1;
  const char* max_m_raw = std::getenv("VLLM_MARLIN_TURING_THREAD_MAX_M");
  if (max_m_raw != nullptr && max_m_raw[0] != '\0') {
    max_m = std::atoi(max_m_raw);
  }

  override_cfg = thread_override_t{thread_k, thread_n, num_threads, max_m};
  return override_cfg;
}
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
  ...
  while (rest_m) {
    ...
    if (thread_k != -1 && thread_n != -1) {
      ...
      auto override_cfg = (major_capability == 7 && minor_capability == 5)
                              ? get_turing_thread_override()
                              : std::nullopt;
      if (override_cfg.has_value() &&
          (override_cfg->max_m < 0 || prob_m_split <= override_cfg->max_m)) {
        thread_tfg = thread_config_t{
            override_cfg->thread_k,
            override_cfg->thread_n,
            override_cfg->num_threads,
        };
        exec_cfg = exec_config_t{1, thread_tfg};
      } else {
      // Auto config
        exec_cfg = determine_exec_config(
            ...
            thread_m_blocks, m_block_size_8, num_bits, group_size,
            has_act_order, is_k_full, has_zp, is_zp_float, is_a_8bit, stages,
            max_shared_mem, sms);
