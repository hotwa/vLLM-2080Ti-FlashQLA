...
class ChunkGatedDeltaRule(CustomOp):
    def __init__(self) -> None:
        ...
        if backend == "flashqla_legacy":
            self._use_flashqla_legacy = True
            self._flashqla_ext = None
            logger.info_once("Using FlashQLA legacy GDN prefill kernel (SM75)")
        ...
        if backend != "flashqla_legacy":
            if supports_flashinfer or backend == "flashinfer":
    ...
    def _get_flashqla_ext(self):
        if self._flashqla_ext is None:
            import importlib.util
            import os
            from pathlib import Path
            # 从当前文件向上找到仓库根目录
            # gdn_linear_attn.py -> mamba -> layers -> model_executor -> vllm -> repo_root
            repo_root = Path(__file__).resolve().parents[4]
            flashqla_path = (repo_root / "FlashQLA-SM70-SM75"
                             / "flash_qla" / "ops" / "gated_delta_rule"
                             / "legacy" / "sm_legacy.py")
            if not flashqla_path.exists():
                # fallback: 尝试环境变量
                flashqla_root = os.environ.get("FLASHQLA_ROOT", "")
                if flashqla_root:
                    flashqla_path = (Path(flashqla_root) / "flash_qla"
                                     / "ops" / "gated_delta_rule"
                                     / "legacy" / "sm_legacy.py")
            spec = importlib.util.spec_from_file_location(
                "flash_qla_legacy_gdn", str(flashqla_path))
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            self._flashqla_ext = mod
        return self._flashqla_ext
    ...
    def forward_flashqla_legacy(
        ...
        mod = self._get_flashqla_ext()

        orig_dtype = q.dtype
        # q/k/v: [B, T, H, D] or [T, H, D] -> [B, T, H, D]
        if q.ndim == 3:
            q_4d = q.unsqueeze(0)
        else:
            q_4d = q
        ...
        q_4d = q_4d.to(torch.float32).contiguous()
        k_4d = k_4d.to(torch.float32).contiguous()
        v_4d = v_4d.to(torch.float32).contiguous()
        g_4d = g_4d.to(torch.float32).contiguous()
        beta_4d = beta_4d.to(torch.float32).contiguous()
        state_4d = (initial_state.unsqueeze(0).to(torch.float32).contiguous()
        ...
        out, final_state = mod.chunk_gated_delta_rule_fwd_legacy(
            q_4d, k_4d, v_4d, g_4d, beta_4d,
            initial_state=state_4d,
        )

        o = out.squeeze(0).to(orig_dtype)
        if core_attn_out is not None:
            o_flat = o.reshape(-1)
            co_flat = core_attn_out.reshape(-1)
            co_flat[: o_flat.numel()].copy_(o_flat)

        final_state_out = final_state.squeeze(0) if output_final_state else None
        return o, final_state_out
